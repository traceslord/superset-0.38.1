...
function drawChart(chart, teamData, teamIndex, propsConfig, propsLabel) {
  ...
  if (!propsConfig.echartsYLeft) propsConfig.echartsYLeft = [];
  // eslint-disable-next-line no-param-reassign
  if (!propsConfig.echartsYRight) propsConfig.echartsYRight = [];
  if (!propsConfig.echartsLegendNotSelected)
    // eslint-disable-next-line no-param-reassign
    propsConfig.echartsLegendNotSelected = [];
